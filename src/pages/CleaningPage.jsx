...
export default function CleaningPage() {
  ...
  const [people, setPeople] = useState([]) // שמירת אנשי הצוות לטובת שיוך אחראי!
  ...
  async function load() {
    ...
    const [areasRes, postsRes, persRes] = await Promise.all([
      ...
      ids.length === 1
        ? supabase.from('personnel').select('id, name, role, unit_id').eq('unit_id', ids[0]).order('name')
        : supabase.from('personnel').select('id, name, role, unit_id').in('unit_id', ids).order('name')
    ])
  ...
  async function addArea(postId, areaName) {
    ...
    if (error) {
      ...
      setCustomNames(prev => ({ ...prev, [postId]: '' }))
    }
  }
  ...
  // פונקציה לעדכון האחראי על הניקיון
                  ...
                  <div className="divide-y divide-border1/50">
                    ...
                    {postAreas.map(area => (
                      <div key={area.id} className="flex flex-col sm:flex-row sm:items-center px-4 py-3 hover:bg-bg2/50 gap-3 sm:gap-2">
                        ...
                        <div className="flex-1 font-bold min-w-0 flex items-center gap-3">
                          <span className="truncate">{area.name}</span>
                        </div>

                        <div className="flex gap-2 items-center w-full sm:w-auto">
                          {/* תפריט בחירת אחראי ניקיון */}
                          <select className="form-input text-xs py-1.5 flex-1 sm:w-32 bg-bg2 border-border2 text-text2 cursor-pointer"
                            value={area.assigned_to || ''} onChange={e => assignPerson(area.id, e.target.value)}>
                            <option value="">-- ללא אחראי --</option>
                            {people.filter(p => p.unit_id === post.unit_id).map(p => (
                              <option key={p.id} value={p.id}>{p.name}</option>
                            ))}
                          </select>
                          
                          <select className={`form-input text-xs font-bold py-1.5 flex-1 sm:w-auto cursor-pointer ${area.status === 'clean' ? 'bg-green-900/30 text-green-400 border-green-500/50' : 'bg-red-900/30 text-red-400 border-red-500/50'}`}
