...
function PersonnelTab() {
  ...
  // יחידה שאפשר לשייך עמדה אליה (לפיקוד/אוגדה — בחירה, ליחידה — רק שלה)
  ...
  async function loadPosts() {
    try {
      ...
      const { data } = await q.order('unit_id,name')
      setPosts(data || [])
    } catch { setPosts([]) }
  }
  ...
  async function save() {
    ...
    await supabase.from('personnel').insert({
      ...
      status: form.status, training_status: form.training_status,
  ...
  async function savePost() {
    ...
    await supabase.from('unit_posts').insert({
      unit_id: targetUnit, name: postForm.name, required: postForm.required || 1
    })
  ...
  const postsWithGap = posts.map(post => {
    const assigned = people.filter(p => p.post_id===post.id && p.status!=='unavailable').length
    return { ...post, assigned, gap: Math.max(0, post.required - assigned) }
  })
  const totalGap = postsWithGap.reduce((s,p) => s+p.gap, 0)

  const filtered = search
    ? people.filter(p => p.name.includes(search) || p.role.includes(search))
    : people
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap justify-between items-center gap-2">
        <div className="flex gap-2">
          <input className="form-input w-44" placeholder="חיפוש שם / תפקיד..."
            value={search} onChange={e=>setSearch(e.target.value)} />
        </div>
        <div className="flex gap-2">
          <button className="btn btn-sm"
            style={{background:'rgba(139,92,246,.15)',borderColor:'rgba(139,92,246,.4)',color:'#a78bfa'}}
      ...
      {/* רשימת אנשים — עיצוב נקי: שם + עמדה + סטטוס */}
      <div className="space-y-1.5">
        {filtered.map(p => {
          ...
          return (
            <div key={p.id}
              ...
              {/* הכשרה */}
              <span className={`badge text-xs flex-shrink-0 ${TR_CLS[p.training_status]}`}>
                {TR_LABEL[p.training_status]}
              </span>
                ...
                {Object.entries(STATUS_ICON).map(([k,icon]) => (
                  <button key={k} title={STATUS_LABEL[k]} onClick={()=>setStatus(p.id,k)}
                    className={`text-sm px-0.5 transition-all leading-none
                      ${p.status===k?'opacity-100 scale-110':'opacity-25 hover:opacity-60'}`}>
                    {icon}
                  </button>
                ))}
              </div>
        ...
        {filtered.length===0 && (
          <div className="card p-8 text-center text-text3">
            {people.length===0 ? 'אין אנשים — לחץ "+ הוסף"' : 'לא נמצאו תוצאות'}
          </div>
        )}
      </div>
      ...
      <Modal open={modal} onClose={()=>setModal(false)} title="➕ הוספת איש צוות">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <select className="form-input" value={form.training_status}
              onChange={e=>setForm(f=>({...f,training_status:e.target.value}))}>
              <option value="none">טרם</option>
              <option value="active">בהכשרה</option>
      ...
      <Modal open={postModal} onClose={()=>setPostModal(false)} title="⚙ ניהול עמדות / מקומות">
        <div className="space-y-4">
          {postsWithGap.length > 0 && (
            <div className="space-y-1.5 max-h-52 overflow-y-auto">
              {postsWithGap.map(post => (
                <div key={post.id}
                  className="flex items-center gap-3 p-2.5 bg-bg3 rounded-xl">
                  <div className="flex-1 min-w-0">
                    <span className="font-bold text-sm">{post.name}</span>
                    {canManageMultiple && (
                      <span className="text-text3 text-xs mr-2">· {unitLabel(post.unit_id)}</span>
                    )}
                  </div>
          ...
          {posts.length===0 && (
            <p className="text-text3 text-sm text-center py-2">אין עמדות מוגדרות</p>
          )}

          <div className="border-t border-border1 pt-4 space-y-2">
            <p className="text-xs text-text3 font-bold">➕ הוסף עמדה חדשה</p>
            <div className="flex gap-2 flex-wrap">
              <input className="form-input flex-1" placeholder="שם (מטבח א׳, מחסן...)"
                value={postForm.name}
              ...
              <input type="number" min="1" max="20" className="form-input w-20" placeholder="נדרש"
                ...
                onChange={e=>setPostForm(f=>({...f,required:parseInt(e.target.value)||1}))} />
            </div>
            {/* לפיקוד/אוגדה — בחר לאיזו יחידה */}
            {canManageMultiple && (
              <select className="form-input w-full" value={postForm.unitId}
                onChange={e=>setPostForm(f=>({...f,unitId:e.target.value}))}>
                <option value="">בחר יחידה לעמדה זו</option>
                {leafUnits.map(u=><option key={u.id} value={u.id}>{u.icon} {u.name}</option>)}
              </select>
            )}
            <button className="btn w-full" onClick={savePost}>הוסף עמדה</button>
            <p className="text-xs text-text3">"נדרש" = כמה אנשים חייבים לכסות עמדה זו</p>
          </div>
        </div>
