...
export default function CommandPage() {
  ...
  async function loadAll() {
    ...
    const [persRes, equipRes, areasRes, incRes, dlRes] = await Promise.all([
      supabase.from('personnel').select('unit_id,training_status,status').in('unit_id', unitIds),
      supabase.from('equipment').select('unit_id,have,need').in('unit_id', unitIds),
      supabase.from('cleaning_areas').select('unit_id,status').in('unit_id', unitIds),
      supabase.from('incidents').select('unit_id,id').eq('status','open').in('unit_id', unitIds),
  ...
  async function sendTask() {
    ...
    const rows = targets.map(uid => ({
      unit_id: uid, title: taskForm.title, description: taskForm.desc,
      priority: taskForm.priority, due_date: taskForm.date || null,
      status: 'todo', assigned_by: senderName
    }))
    await supabase.from('tasks').insert(rows)
    showToast(`משימה נשלחה ל${taskForm.target ? UNITS.find(u=>u.id===taskForm.target)?.name : 'כל היחידות תחתי'} ✅`, 'green')
    setTaskModal(false)
  ...
  async function saveDispatch() {
    ...
    const { error } = await supabase.from('dispatch_log').insert({
      ...
      quantity: dispForm.qty, notes: dispForm.note, dispatched_by: currentUnit?.name
    })
    if (!error) {
      ...
      if (existing) {
        ...
        await supabase.from('equipment').insert({
          unit_id: dispForm.unit, name: dispForm.item,
  ...
  async function exportPDF() {
    ...
    const html = `<!DOCTYPE html><html dir="rtl" lang="he"><head><meta charset="UTF-8"><title>דוח מצב — ${date}</title>
<style>body{font-family:Arial;background:#111;color:#e0e0e0;padding:40px;direction:rtl}
h1{color:#f5c842;border-bottom:2px solid #f5c842;padding-bottom:10px}
table{width:100%;border-collapse:collapse;background:#1a1a2e}th{background:#1e3a5f;padding:10px;color:#93c5fd}
.kpi{display:flex;gap:20px;flex-wrap:wrap;margin:20px 0}.kpi-box{background:#1a1a2e;border:1px solid #333;border-radius:8px;padding:16px 24px;text-align:center}
.kpi-val{font-size:2em;font-weight:bold;color:#f5c842}.kpi-lbl{color:#888;font-size:.85em;margin-top:4px}</style></head><body>
<h1>⭐ דוח מצב — ${currentUnit?.name}</h1>
<p style="color:#888">${date} | ${daysLeft} ימים לפסח</p>
<div style="text-align:center;margin:20px 0"><div style="font-size:3em;font-weight:bold;color:${readiness>=80?'#4ade80':readiness>=60?'#fb923c':'#f87171'}">${readiness}%</div><div style="color:#888">מוכנות</div></div>
<div class="kpi">
  <div class="kpi-box"><div class="kpi-val">${avgTrained}%</div><div class="kpi-lbl">הכשרה</div></div>
  <div class="kpi-box"><div class="kpi-val">${avgClean}%</div><div class="kpi-lbl">ניקיון</div></div>
  ...
  <div class="kpi-box"><div class="kpi-val" style="color:${totalInc===0?'#4ade80':'#f87171'}">${totalInc}</div><div class="kpi-lbl">חריגים</div></div>
</div>
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          <button onClick={()=>setBriefing(true)} className="btn text-xs px-3 py-2 bg-purple-900/40 border-purple-500/50 text-purple-300">🖥 הערכת מצב</button>
          ...
          <button onClick={exportPDF} className="btn btn-sm bg-green-900/30 border-green-500/40 text-green-400">📄 ייצא PDF</button>
        </div>
      </div>
        ...
        <div className="space-y-2">
          {criticalAlerts.slice(0,3).map(a=>(
            <div key={a.id} className="bg-red-900/20 border border-red-500/40 rounded-xl px-4 py-2.5 flex items-center gap-3">
              ...
              <span className="text-red-300 text-sm font-bold flex-1">{a.message}</span>
              {a.unit && <span className="text-red-400/70 text-xs">— {a.unit}</span>}
            </div>
          ))}
        </div>
      )}
      ...
      <div className="card p-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            { label:'ציוד', icon:'📦',
              val: totals.every(s=>s.personnel===0) ? 0 : totalMissing===0?100:Math.max(0,100-totalMissing*8),
              color: totals.every(s=>s.personnel===0)?'bg-border2':totalMissing===0?'bg-green-500':totalMissing<=3?'bg-orange-500':'bg-red-500',
              text: totals.every(s=>s.personnel===0)?'אין נתונים':totalMissing===0?'מלא':`חסר ${totalMissing}` },
            { label:'חריגים', icon:'🆘',
              val: totals.every(s=>s.personnel===0) ? 0 : totalInc===0?100:Math.max(0,100-totalInc*15),
              color: totals.every(s=>s.personnel===0)?'bg-border2':totalInc===0?'bg-green-500':totalInc<=2?'bg-orange-500':'bg-red-500',
              text: totals.every(s=>s.personnel===0)?'אין נתונים':totalInc===0?'נקי':`${totalInc} פתוחים` },
          ].map(item=>(
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border1 text-text3 text-xs">
                <th className="text-right p-3 font-bold">יחידה</th>
                <th className="text-center p-3 font-bold">הכשרה</th>
                <th className="text-center p-3 font-bold">ניקיון</th>
                <th className="text-center p-3 font-bold">ציוד חסר</th>
                <th className="text-center p-3 font-bold">חריגים</th>
                <th className="text-center p-3 font-bold">מצב</th>
              </tr>
            </thead>
                  ...
                  <tr key={u.id} className="hover:bg-bg3/50 transition-colors">
                    <td className="p-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div><div className="font-bold">{u.name}</div><div className="text-text3 text-xs">{s.personnel||0} אנשים</div></div>
                      </div>
                    </td>
                    ...
                    <td className="p-3 text-center">
                      <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-lg text-xs font-bold border ${healthColor[s.health||'gray']}`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${healthDot[s.health||'gray']}`}/>
                        {healthLabel[s.health||'gray']}
                      </span>
                    </td>
                  </tr>
                )
      ...
      {viewMode==='compare' && (
        <div className="space-y-3">
          {nonAdminUnits.map(u => {
            const s = unitStats[u.id] || {}
            return (
              <div key={u.id} className={`card p-4 border-2 ${healthColor[s.health||'gray']}`}>
                <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                  <div className="flex items-center gap-2">
                    <span className="text-xl">{u.icon}</span>
                    <div><div className="font-black">{u.name}</div><div className="text-text3 text-xs">{u.brigade}</div></div>
                  </div>
                  <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold border ${healthColor[s.health||'gray']}`}>
                    <span className={`w-2 h-2 rounded-full ${healthDot[s.health||'gray']}`}/>
                    {healthLabel[s.health||'gray']}
                  </span>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
      ...
      <Modal open={taskModal} onClose={()=>setTaskModal(false)} title="📋 שליחת משימה ליחידות">
        <div className="space-y-3">
          <div><label className="text-xs text-text3 font-bold block mb-1">כותרת</label><input className="form-input" value={taskForm.title} onChange={e=>setTaskForm(f=>({...f,title:e.target.value}))}/></div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div><label className="text-xs text-text3 font-bold block mb-1">תאריך יעד</label>
              <input type="date" className="form-input" value={taskForm.date} onChange={e=>setTaskForm(f=>({...f,date:e.target.value}))}/></div>
          </div>
          <div><label className="text-xs text-text3 font-bold block mb-1">יחידת יעד</label>
            <select className="form-input" value={taskForm.target} onChange={e=>setTaskForm(f=>({...f,target:e.target.value}))}>
              <option value="">כל היחידות תחתי</option>
              {nonAdminUnits.map(u=><option key={u.id} value={u.id}>{u.icon} {u.name}</option>)}
            </select></div>
        </div>
      ...
      <Modal open={dispatchModal} onClose={()=>setDispatchModal(false)} title="📦 ניפוק ציוד">
        <div className="space-y-3">
          <div><label className="text-xs text-text3 font-bold block mb-1">יחידה</label>
            <select className="form-input" value={dispForm.unit} onChange={e=>setDispForm(f=>({...f,unit:e.target.value}))}>
              <option value="">בחר יחידה</option>
              {nonAdminUnits.map(u=><option key={u.id} value={u.id}>{u.icon} {u.name}</option>)}
            </select></div>
          <div><label className="text-xs text-text3 font-bold block mb-1">פריט</label>
            <input className="form-input" placeholder="לדוג: קנקן, אנכיה..." value={dispForm.item} onChange={e=>setDispForm(f=>({...f,item:e.target.value}))}/></div>
          <div className="grid grid-cols-2 gap-3">
            <div><label className="text-xs text-text3 font-bold block mb-1">כמות</label>
              <input type="number" min="1" className="form-input" value={dispForm.qty} onChange={e=>setDispForm(f=>({...f,qty:parseInt(e.target.value)||1}))}/></div>
            <div><label className="text-xs text-text3 font-bold block mb-1">הערות</label>
              <input className="form-input" value={dispForm.note} onChange={e=>setDispForm(f=>({...f,note:e.target.value}))}/></div>
          </div>
        </div>
