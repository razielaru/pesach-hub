...
import { UNITS, getSubordinateUnits, getLeafUnits } from '../lib/units'
...
export default function CommandPage() {
  ...
  // יחידות-עלה בלבד (לא אוגדות) — לטבלה, משימות, ניפוק
    ...
    <div className="space-y-5">
          ...
          {criticalAlerts.slice(0,3).map(a=>(
            <div key={a.id} className="bg-red-900/20 border border-red-500/40 rounded-xl px-4 py-2.5 flex items-center gap-3">
              ...
              <div className="flex-1">
                <span className="text-red-300 text-sm font-bold">{a.message}</span>
                {a.unit && <span className="text-red-400/70 text-xs mr-2">— {a.unit}</span>}
              </div>
            </div>
          ))}
        </div>
      ...
      {viewMode === 'table' && (
        <div className="card overflow-x-auto">
          <div className="panel-head">
            <span className="panel-title">📋 מצב יחידות — {currentUnit?.name}</span>
          </div>
                    ...
                    <td className="p-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <div className="text-text3 text-xs">{s.personnel||0} אנשים</div>
                        </div>
                      </div>
                    </td>
                    <td className="p-3 text-center">
                      <span className={`font-bold ${s.trainedPct>=70?'text-green-400':s.trainedPct>=40?'text-orange-400':'text-red-400'}`}>
                        {s.trainedPct||0}%
                      </span>
                    </td>
                    <td className="p-3 text-center">
                      <span className={`font-bold ${s.cleanPct>=70?'text-green-400':s.cleanPct>=40?'text-orange-400':'text-red-400'}`}>
                        {s.cleanPct||0}%
                      </span>
                    </td>
                    <td className="p-3 text-center">
                      <span className={`font-bold ${s.equipMissing===0?'text-green-400':'text-red-400'}`}>
                        {s.equipMissing||0}
                      </span>
                    </td>
                    <td className="p-3 text-center">
                      <span className={`font-bold ${s.openInc===0?'text-green-400':'text-red-400'}`}>
                        {s.openInc||0}
                      </span>
                    </td>
                    <td className="p-3 text-center">
                      <span className={`inline-flex items-center gap-1.5 px-2 py-1 rounded-lg text-xs font-bold border ${healthColor[s.health||'green']}`}>
                        <span className={`w-1.5 h-1.5 rounded-full ${healthDot[s.health||'green']}`}/>
                        {healthLabel[s.health||'green']}
                      </span>
                    </td>
                  </tr>
                )
      ...
      {viewMode === 'compare' && (
        <div className="space-y-3">
          {nonAdminUnits.map(u => {
            const s = unitStats[u.id] || {}
            const brigadeUnits = nonAdminUnits.filter(x => x.brigade === u.brigade)
            const brigadeAvg = brigadeUnits.length
              ? Math.round(brigadeUnits.reduce((a,x)=>(a+(unitStats[x.id]?.trainedPct||0)),0)/brigadeUnits.length) : 0
            return (
                ...
                <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <div className="text-text3 text-xs">{u.brigade}</div>
                    </div>
                  </div>
                  <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold border ${healthColor[s.health||'green']}`}>
                    <span className={`w-2 h-2 rounded-full ${healthDot[s.health||'green']}`}/>
                    {healthLabel[s.health||'green']}
                  </span>
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
      ...
      <Modal open={taskModal} onClose={()=>setTaskModal(false)} title="📋 שליחת משימה ליחידות">
        <div className="space-y-3">
          <div><label className="text-xs text-text3 font-bold block mb-1">כותרת המשימה</label>
            <input className="form-input" value={taskForm.title} onChange={e=>setTaskForm(f=>({...f,title:e.target.value}))}/></div>
          <div><label className="text-xs text-text3 font-bold block mb-1">תיאור</label>
            <input className="form-input" value={taskForm.desc} onChange={e=>setTaskForm(f=>({...f,desc:e.target.value}))}/></div>
