...
// Cache logos in memory so they don't reload on every navigation
const LOGO_CACHE = {}
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    loadStats()
    // Load logo separately only if not cached
    if (!LOGO_CACHE[currentUnit.id]) {
      loadLogo()
    }
    const ch = supabase.channel('incidents_dash')
      .on('postgres_changes', { event:'INSERT', schema:'public', table:'incidents', filter:`unit_id=eq.${currentUnit.id}` }, () => loadStats())
      .subscribe()
  ...
  async function loadStats() {
    ...
    // Load stats WITHOUT logo_url — much faster
    const [pers, equip, areas, openTasks, openInc] = await Promise.all([
      ...
      supabase.from('tasks').select('id,title,priority,status,assigned_by').eq('unit_id', uid).neq('status','done').order('created_at',{ascending:false}).limit(4),
      supabase.from('incidents').select('id,title,severity').eq('unit_id', uid).eq('status','open').order('created_at',{ascending:false}).limit(3),
    ])
  ...
  async function loadLogo() {
    // Load only logo_url — separate lightweight query
    const { data } = await supabase.from('units').select('logo_url').eq('id', currentUnit.id).single()
    if (data?.logo_url) {
      LOGO_CACHE[currentUnit.id] = data.logo_url
      setUnitLogo(data.logo_url)
    }
  }
    ...
    <div className="space-y-5">
      ...
      <div className="card p-6 bg-gradient-to-l from-[#1a2040] to-bg2 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          {/* Logo — shows icon immediately, logo when loaded */}
        ...
        <div className="text-center bg-bg3 rounded-2xl px-8 py-4 border border-border2 flex-shrink-0">
      ...
      <div className="grid md:grid-cols-2 gap-5">
        ...
        <div className="card">
          <div className="panel-head"><span className="panel-title">📊 סטטוס מהיר</span></div>
          <div className="p-5 space-y-4">
            {[
              ...
              <div key={item.label}>
                <div className="flex justify-between text-xs text-text2 mb-1"><span>{item.label}</span><span>{item.val}%</span></div>
