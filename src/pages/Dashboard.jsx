...
export default function Dashboard() {
  ...
  async function loadStats() {
    try {
      ...
      const [pers, equip, areas, openTasks, openInc, postsRes, lastPers, lastInc, lastClean] = await Promise.all([
        ...
        (ids.length === 1
          ? supabase.from('unit_posts').select('id, unit_id, required').eq('unit_id', ids[0])
          : supabase.from('unit_posts').select('id, unit_id, required').in('unit_id', ids)
        ),
        inF(supabase.from('personnel').select('unit_id, created_at').order('created_at',{ascending:false}).limit(50)),
        inF(supabase.from('incidents').select('unit_id, created_at').order('created_at',{ascending:false}).limit(50)),
  ...
  const trainedPct = stats.total ? Math.round(stats.trained/stats.total*100) : 0
  if (stats.total > 0 && trainedPct < 50) localCriticalAlerts.push({ icon: '🎓', message: `פער קריטי בהכשרות (כרגע ${trainedPct}%)` })
  if (stats.equipTotal > 0 && stats.equipPct < 40) localCriticalAlerts.push({ icon: '📦', message: `פער קריטי בציוד (כרגע ${stats.equipPct}%)` })
      ...
      <div className="grid md:grid-cols-2 gap-5">
        ...
        <div className="card">
          ...
          <div className="p-5 space-y-4">
            ...
            {((stats.equipTotal > 0 && stats.equipPct < 40) || (stats.total > 0 && Math.round(stats.trained/stats.total*100) < 50)) && (
