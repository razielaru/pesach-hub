...
create table if not exists chat_messages (
  ...
  channel_name text default 'כללי', -- כללי | לוגיסטיקה | כשרות | דחוף
...
-- שיבוצי ליל הסדר
create table if not exists seder_assignments (
  id uuid primary key default gen_random_uuid(),
  unit_id text references units(id) on delete cascade,
...
alter table seder_assignments enable row level security;

create policy "public all" on chat_messages for all using (true);
create policy "public all" on broadcast_alerts for all using (true);
create policy "public all" on seder_assignments for all using (true);

-- חטיבות היתר — הוסף ל-units אם לא קיים
insert into units (id, name, icon, brigade, pin, is_admin, is_senior)
values ('hativa_other', 'חטיבות היתר', '⚔', 'חטיבות', null, false, false)
on conflict (id) do nothing;
...
create policy "public all" on public.training_videos for all using (true);
